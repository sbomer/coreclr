...
#endif
    {
        pCell->SetDelayLoadHelper(m_pImage->GetImportTable()->GetPlacedIndirectHelperThunk(CORINFO_HELP_EE_EXTERNAL_FIXUP));
    }
...
DWORD ZapIndirectHelperThunk::SaveWorker(ZapWriter * pZapWriter)
{
    ...
    *p++ = 0x25;
    if (pImage != NULL) {
#ifdef _TARGET_x86_
        pImage->WriteReloc(buffer, (int)(p - buffer), pImage->GetHelperThunk(GetHelper()), 0, IMAGE_REL_BASED_PTR);
#else
        // _TARGET_AMD64_
